...
\begin{itemize}
\item \code{time}:
	percentage of time the function has in the program (based on "\code{self}")
\item \code{cumu}:
...
\item \code{calls}:
	...
	optionally appended "+" to former number (see function \code{c} in example output above).
\item \code{ms/call}:
...
Every function has a unique index number \code{i} (here ``1''), based on the position in the
...
If \code{a} would call another function \code{d} and \code{d} would call \code{a}, all three
...
\begin{itemize}
	...
	\item \code{calls}: the number of calls to the member function from within the cycle
\end{itemize}

External functions (here \code{c}):
\begin{itemize}
	\item the meaning of the fields are the same as with the member functions, except\ldots
	\item \code{calls}: the number of calls to the external function from the cycle
		and
...
runtime shorter than the interrupt time are likely to be profiled inaccurate or may not be seen at all.

The default interrupt time can be set with the \code{profiler-interrupt-time} command processor setting,
see \ref{settings}. For example, \code{,set profiler-interrupt-time 200} sets the
...
Therefore it schedules the {\tt alarm}-interrupt at a regular interval specified by the \code{profiler-interrupt-time}
command processor setting. Because the thread system uses the alarm interrupt as well, the profiler interrupt handler
...
down to the continuation of the \code{profile-thunk} function, which called the thunk to be profiled.
...
\begin{itemize}
	\item if \code{gc-count} increased, distribution can not be done
