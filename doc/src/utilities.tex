...
\code{Enqueue!} adds a value to the queue.
\code{Dequeue!} and \code{maybe-dequeue!} remove a value from the queue if
 one is available; if the queue is empty, \code{dequeue!} raises an error,
 and \code{maybe-dequeue!} returns \code{\#f}.
\code{Maybe-dequeue!} can therefore be used to avoid a race condition
 between \code{queue-empty?} and \code{dequeue!} if a queue known to never
 contain \code{\#f} must be read by two or more threads at once.
...
\end{protos}
\noindent
\code{Queue-head} returns the first value in \var{queue} if it is not empty,
 or raises an error if it is empty.
...
\code{Delete-from-queue!} removes the first instance of \var{value} from
 \var{queue}, using \code{eq?} for comparisons.
...
\code{On-queue?} returns \code{\#t} if \var{value} is in the \var{queue}
 (using \code{eq?} for comparisons) and \code{\#f} if it is not.
