...
All functions need a \code{profile-data} record argument, which can be created with
\code{make-empty-profile-data}.

\begin{protos}
	\proto{make-empty-profile-data}{}{profile-data}
	\proto{profile-thunk}{profile-data thunk}{value}
	\proto{profile-thunk}{profile-data thunk interrupt-time}{value}
\end{protos}

With \code{profile-thunk} a thunk is run under the profiler. The data is stored in
the profile-data record given as argument. Optionally the timeout for the profiler interrupt
can be given (in milliseconds). The return value of \code{profile-thunk} is the return value
of \code{thunk}.
...
The profiling data structures for single functions or cycles can be
retrieved by the \code{profile-data-templates} and \code{profile-data-cycles}
tables.

The following functions produce the same output as the \code{,profile} command.
They all take a optional port argument. Default is the \cvar{current-output-port}.

\begin{protos}
	\protonoresult{profile-display}{ profile-data}
	\protonoresult{profile-display}{ profile-data port}
	\protonoresult{profile-display-overview}{ profile-data}
	\protonoresult{profile-display-overview}{ profile-data port}
	\protonoresult{profile-display-flat}{ profile-data}
	\protonoresult{profile-display-flat}{ profile-data port}
	\protonoresult{profile-display-tree}{ profile-data}
...
Here is \cvar{names} the list of names specifying the function, optionally
...
\cvar{names} can also be a plain string, as in
\code{profile-display-function-flat prof-data "module"}. This will display
all flat function profiles which have ``module'' either as name or module.

The meanings of the fields the functions return are as follows:
\begin{itemize}
	\item \code{calls}:
