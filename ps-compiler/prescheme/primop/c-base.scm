...
(define-c-generator jump #f
  (lambda (call port indent)
    (let ((proc (called-lambda call)))
      ...
      (if (and (call-node? (lambda-body proc))
               (eq? (primop-id (call-primop (lambda-body proc))) 'COMPUTED-GOTO))
        (write-jump-to-computed-goto proc port indent)
        (begin
...
(define (write-jump-to-computed-goto proc port indent)
  (let* ((arg-vars  (lambda-variables proc))
         (call      (lambda-body proc))
         (size      (call-exits call))
         (parent-id (lambda-id proc)))
    ...
    (format port "];~%#else~%")
    (indent-to port indent)
    (format port "goto L~D;~%#endif~%" parent-id)))
